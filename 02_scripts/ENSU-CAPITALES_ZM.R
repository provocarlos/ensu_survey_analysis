...
files_temp <- files[(grep("0321|0621|0921|1221|0322|0622|0922|1222", files, ignore.case = T))];
  ...
  temp %>% 
    mutate(
      ...
      var_gdl = case_when(
        ...
        cd == 64 ~ "Monterrey",
        cd %in% as.character(71:86) ~ "Ciudad de México",
        T ~ "Otro")
    ) %>% 
    select(var_id, var_gdl, upm_dis, 
           est_dis, fac_sel, bp1_1) %>% 
...
ensu_full %>% 
  ...
  mutate(
    trim = as.yearqtr(fecha),
    alphavar = case_when(var_id %in% c("Ciudad de México", "Guadalajara", "Monterrey") ~ "Color",
                         T ~ "NoColor")
  ) -> df_percp
...
ggplot(
  ...
  scale_y_continuous("% de población", labels = scales::percent, limits = c(0, 1),
